...
namespace EventTools
{
    public sealed class Config : IConfig
    {
        ...
        [Description("The broadcast to be sent when using the EventNext command. Variables: EVENTNAME")]
        ...
        [Description("Whether or not the EventNext command should send a message to discord. Default: false.")]
        public bool ENextSendToDiscord { get; set; } = false;

        [Description("The message to be sent via a discord webhook when using the EventNext command. Variables: EVENTNAME, SERVERNAME, DISCORDMENTION")]
        public string ENextDiscordMessage { get; set; } = "{DISCORDMENTION} In the next round there will be an event: `{EVENTNAME}` on the server: `{SERVERNAME}`";

        [Description("Discord ID of the role to mention when using the EventNext command.")]
        public string ENextDiscordRoleID { get; set; } = "";

        [Description("Name of the Discord Webhook used in the EventNext command")]
        public string ENextWebhookName { get; set; } = "EventNotifier";

        [Description("The Discord Webhook URL used in the EventNext command.")]
        ...
        [Description("The message to be broadcast when using the EventNow command. Variables: EVENTNAME")]
        public string ENowBC { get; set; } = "<size=40><b><color=#cc9900>Event:\n</color></b></size> <size=50><b><color=#ff0000>{EVENTNAME}</color></b></size>";

        [Description("Whether or not the EventNow command should send a message to discord. Default: false.")]
        ...
        [Description("Name of the Discord Webhook used in the EventNow command.")]
        public string ENowWebhookName { get; set; } = "EventNotifier";

        [Description("The Discord Webhook URL used in the EventNow command.")]
        ...
        [Description("The broadcast to be sent whenever the lottery command is used. Variables: NUMBER")]
